...
#if defined(CONFIG_FB_S3C_WA101S) || defined(CONFIG_FB_S3C_LTE480WV) || defined(CONFIG_FB_S3C_S6C1372) || defined(CONFIG_FB_S3C_S6F1202A)
void s3cfb_cfg_gpio(struct platform_device *pdev)
{
	int i;
	u32 reg;
...
int s3cfb_mdnie_clk_on(void)
{
	...
	rate = 73000000;
#elif defined(CONFIG_FB_S3C_S6E8AB0)
	rate = 80000000;
#else
...
int s3cfb_mdnie_pwm_clk_on(void)
{
	...
	printk(KERN_INFO "set mdnie_pwm_pre sclk rate to %d\n", rate);
...
int s3cfb_clk_on(struct platform_device *pdev, struct clk **s3cfb_clk)
{
	...
	rate = 80000000;
#else
	rate = clk_round_rate(sclk, 90000000);
...
int s3cfb_backlight_on(struct platform_device *pdev)
{
	...
	err = gpio_request(GPIO_LED_BACKLIGHT_RESET,
			"GPIO_LED_BACKLIGHT_RESET");
	if (err) {
		printk(KERN_ERR	"failed	to request RESET for lcd backlight control\n");
		return err;
	}
