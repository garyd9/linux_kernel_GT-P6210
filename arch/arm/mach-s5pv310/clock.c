...
static struct clksrc_clk clksrcs[] = {
	{
		...
		.sources = &clkset_group,
		.reg_src = { .reg = S5P_CLKSRC_LCD0, .shift = 8, .size = 4 },
		.reg_div = { .reg = S5P_CLKDIV_LCD0, .shift = 12, .size = 4 },
	}, {
		.clk		= {
			.name		= "sclk_mdnie_pwm_pre",
			.id		= 1,
		...
		.sources = &clkset_group,
		.reg_src = { .reg = S5P_CLKSRC_LCD1, .shift = 8, .size = 4 },
		.reg_div = { .reg = S5P_CLKDIV_LCD1, .shift = 12, .size = 4 },
#endif
	}, {
		.clk		= {
			.name		= "sclk_mipi",
...
static int s5pv310_epll_set_rate(struct clk *clk, unsigned long rate)
{
	...
	do {
		...
		if (timeout == 0) {
			printk(KERN_ERR "EPLL failed pll locking.\n");
			return -ETIMEDOUT;
		}
		timeout--;
		udelay(1);
...
static struct vpll_div_data vpll_div[] = {
	...
	{ 160000000, 3, 79, 2, 1024, 0, 31, 1 },
	{ 266000000, 3, 66, 1,  512, 0, 11, 1 },
	{ 300000000, 2, 50, 1,    0, 1,  6, 1 },
...
static int s5pv310_vpll_set_rate(struct clk *clk, unsigned long rate)
{
	...
	timeout = 1000;
	do {
		tmp = __raw_readl(S5P_VPLL_CON0);

		if (timeout == 0) {
			printk(KERN_ERR "VPLL failed pll locking.\n");
			return -ETIMEDOUT;
		}
		timeout--;
		udelay(1);

	} while (!(tmp & (0x1 << S5P_VPLLCON0_LOCKED_SHIFT)));
