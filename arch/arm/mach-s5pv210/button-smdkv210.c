...
{
	if (irq == IRQ_EINT(0))
		printk(KERN_INFO "EINT 0 Button Interrupt occure\n");
	else if (irq == IRQ_EINT(31))
		printk(KERN_INFO "EINT 31 Button Interrupt occure\n");
	else
...
{
	printk(KERN_INFO"SMDKV210 Button init function\n");
	...
	set_irq_type(gpio_to_irq(S5PV210_GPH3(7)), IRQ_TYPE_EDGE_FALLING);

	setup_irq(gpio_to_irq(S5PV210_GPH3(7)), &s3c_button_irq);

#ifdef CONFIG_PM
	set_irq_wake(gpio_to_irq(S5PV210_GPH3(7)), 1);
#endif
