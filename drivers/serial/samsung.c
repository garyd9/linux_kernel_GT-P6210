...
#define	CONFIG_TTY_LOW_LATENCY_PUSH
#if defined(CONFIG_TTY_LOW_LATENCY_PUSH)
...
#define SEC_KEYBOARD (1 << 5)
static bool is_keyboard(struct uart_port *port)
{
	if (NULL == port->cons)
		return false;
	else if ((port->cons->index == port->line) &&
			port->cons->flags & SEC_KEYBOARD)
		return true;
...
#ifdef CSR_GPS_DEBUG
#define GPIO_GPS_DEBUG_NEW S5PV310_GPK1(2)
#endif
	...
	if (received_data_count)
	{
		...
		if (!is_keyboard(port)) {
			...
			if (port->line == CONFIG_BT_S3C_UART)
				queue_work(tty_push_wq, &tty_push_work);
			else
#endif
...
static void s3c24xx_serial_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	...
	else if (port->line == CONFIG_DEBUG_S3C_UART) {
		umcon &= ~S3C2410_UMCOM_AFC;
		wr_regl(port, S3C2410_UMCON, umcon);
	}
...
static ssize_t s3c24xx_serial_rts_pullup_detect(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	back_pull = S3C_GPIO_PULL_NONE; //back_pull = s3c_gpio_getpull(GPIO_GPS_RTS); //back up
	back_cfg = S3C_GPIO_SFN(2); //back_cfg = s3c_gpio_getcfg(GPIO_GPS_RTS);
...
#if defined(CONFIG_TTY_LOW_LATENCY_PUSH)
static void tty_low_latency_push(struct work_struct *work)
{
	struct s3c24xx_uart_port *btport;
	struct uart_port *port;
	struct tty_struct *tty;

	btport = &s3c24xx_serial_ports[CONFIG_BT_S3C_UART];
	port = &btport->port;
...
#if defined(CONFIG_TTY_LOW_LATENCY_PUSH)
{
	struct uart_port *port = &ourport->port;
	if (port->line == CONFIG_BT_S3C_UART) {
		INIT_WORK(&tty_push_work, tty_low_latency_push);
		tty_push_wq = create_singlethread_workqueue("bt_tty_wq");
		if (tty_push_wq == NULL) {
			ret = -ENOMEM;
			pr_err("%s: Failed to create workqueue\n", __func__);
			destroy_workqueue(tty_push_wq);
		}
	}
}
#endif
...
{
	if (NULL == cons_uart)
		pr_err("[Keyboard] cons_uart is NULL\n");
	else if (is_keyboard(cons_uart))
		s3c24xx_serial_console_putchar(cons_uart, val);
}
...
int change_console_baud_rate(int baud)
{
	...
	if (9600 == baud) {
		uart_parse_options("9600", &baud, &parity, &bits, &flow);
		cons_uart->cons->flags |= SEC_KEYBOARD;
	} else {
		uart_parse_options("115200", &baud, &parity, &bits, &flow);
		cons_uart->cons->flags &= ~SEC_KEYBOARD;
	}
	uart_set_options(cons_uart, cons_uart->cons, baud, parity, bits, flow);
