...
static void smb347_charger_init(struct smb347_chg_data *chg)
{
	...
	/* Charge curr : Fast-chg 2200mA, Pre-charge curr 250mA, Term curr 250mA */
	smb347_i2c_write(chg->client, SMB347_CHARGE_CURRENT, 0xDD);

	/* Pin enable control : Charger enable control EN Pin - Active Low */
	/* Pin enable control : USB5/1/HC or USB9/1.5/HC Control - Register Control */
	/* Pin enable control : USB5/1/HC Input state - Tri-state Input */
	...
	if ((system_rev >= 2) && (system_rev <= 5))
		smb347_i2c_write(chg->client, SMB347_SYSOK_USB30_SELECTION, 0x09);	/* Active high */
	else
		smb347_i2c_write(chg->client, SMB347_SYSOK_USB30_SELECTION, 0x08);	/* Active low */
	...
	smb347_i2c_write(chg->client, SMB347_LIMIT_CELL_TEMPERATURE_MONITOR, 0x01);
...
static void smb347_set_charging_state(int enable, int charging_mode)
{
	...
	if (enable) {
		...
		case CABLE_TYPE_TA:
			...
			pr_info("%s : Charging enable, charging current set as HC.", __func__);
			break;
		case CABLE_TYPE_USB:
			/* CommandB : USB5 */
			smb347_i2c_write(chg->client, SMB347_COMMAND_B, 0x02);
			pr_info("%s : Charging enable, charging current set as LOW(USB5).", __func__);
			break;
		default:
			/* CommandB : USB1 */
			smb347_i2c_write(chg->client, SMB347_COMMAND_B, 0x00);
			pr_info("%s : Charging enable, charging current set as LOW(USB1).", __func__);
			break;
		}
...
int smb347_get_charging_current(void)
{
	...
	switch(data >> 5) {
	case 0:	get_current = 700;	break;
	case 1:	get_current = 900;	break;
	case 2:	get_current = 1200;	break;
	case 3:	get_current = 1500;	break;
	case 4:	get_current = 1800;	break;
	case 5:	get_current = 2000;	break;
	case 6:	get_current = 2200;	break;
	case 7:	get_current = 2500;	break;
	default:
		get_current = 700;
		break;
	}
