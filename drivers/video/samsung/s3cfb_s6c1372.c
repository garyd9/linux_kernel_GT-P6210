...
{
	printk("%s: %d", __func__, power);

	return 0;
}

static struct lcd_ops s6c1372_lcd_ops = {
...
static ssize_t lcdtype_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/*For P2*/
	if (lcdtype == 0)
		return sprintf(buf, "HYDIS_NT51008\n");
	else if (lcdtype == 2)
		return sprintf(buf, "BOE_NT51008\n");
	else
		return sprintf(buf, "SMD_S6F1202A02\n");
#else
	/*For p4*/
	return sprintf(buf, "SMD_S6C1372\n");
#endif
}
...
static int __init s6c1372_probe(struct platform_device *pdev)
{
	...
	lcd->ld = lcd_device_register("lcd", &pdev->dev, lcd, &s6c1372_lcd_ops);
