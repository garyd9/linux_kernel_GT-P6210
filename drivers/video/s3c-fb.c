...
/* writeback */
struct s3c_fb_fimd_desc {
	struct s3c_fb	*sfb_dev;
};
...
#ifdef CONFIG_VCM
	struct	s3c_fb_vcm s3c_fb_vcm[S3C_FB_MAX_WIN];
	struct vcm_res		*s5p_vcm_res;
#endif
#ifdef CONFIG_UMP_VCM_ALLOC
	ump_dd_handle		ump_wrapped_buffer[S3C_FB_MAX_WIN];
#endif
};
 ...
 * @sd: The subdevice used by fimd driver for writeback.
 * @slock: The spinlock protectiong this data sturcture.
...
/* writeback */
struct s3c_fb_fimd_desc		*fbfimd;

void s3c_fb_set_output_writeback(struct s3c_fb *sfb)
{
	void __iomem *regs = sfb->regs;
	u32 vidcon0,  vidcon2;

	vidcon0 = readl(regs + VIDCON0);
	vidcon0 &= ~VIDCON0_VIDOUT_MASK;

	vidcon0 |= VIDCON0_VIDOUT_WB_RGB;
	writel(vidcon0, regs + VIDCON0);

	vidcon2 = readl(regs + VIDCON2);
	vidcon2 &= ~(VIDCON2_WB_MASK | VIDCON2_TVFORMATSEL_MASK | \
					VIDCON2_TVFMTSEL1_MASK);

	vidcon2 |= (VIDCON2_WB_ENABLE | VIDCON2_TVFMTSEL_SW | \
				VIDCON2_TVFMTSEL1_YUV444);

	writel(vidcon2, regs + VIDCON2);
}

void s3c_fb_set_output_rgb(struct s3c_fb *sfb)
{
	void __iomem *regs = sfb->regs;
	u32 vidcon0,  vidcon2;
...
static long s3c_fb_wb_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct s3c_fb *sfb = fbfimd->sfb_dev;
	unsigned int *wb_on = (unsigned int *)arg;
	unsigned long flags;

	spin_lock_irqsave(&sfb->slock, flags);

	if (*wb_on)
		s3c_fb_set_output_writeback(sfb);
	else
		s3c_fb_set_output_rgb(sfb);

	spin_unlock_irqrestore(&sfb->slock, flags);
	return 0;
}

static struct v4l2_subdev_core_ops s3c_fb_core_ops = {
	.ioctl  = s3c_fb_wb_ioctl,
};

static struct v4l2_subdev_ops s3c_fb_subdev_ops = {
	.core   = &s3c_fb_core_ops,
};

void s3c_fb_draw_logo(struct fb_info *fb)
{
...
#ifdef CONFIG_VCM
static const struct s5p_vcm_driver s3c_fb_vcm_driver = {
...
#if MALI_USE_UNIFIED_MEMORY_PROVIDER
int s3c_fb_unmap_video_memory(struct s3c_fb *sfb, struct fb_info *fb)
{
	struct fb_fix_screeninfo *fix = &fb->fix;
	unsigned int buffer_size;
	struct s3c_fb_win *win = fb->par;

	buffer_size = PAGE_ALIGN(fix->smem_len);

	if (fix->smem_start) {

		dma_free_writecombine(sfb->dev, buffer_size,
					fb->screen_base, fix->smem_start);

		fix->smem_start = 0;
		fix->smem_len = 0;
		dev_info(sfb->dev, "[fb%d] video memory released\n", \
							win->index);
	}
	return 0;
}

int s3c_fb_ump_wrapper(struct fb_fix_screeninfo *fix, int arg, int id,
			struct s3c_fb_win *win)
{
	ump_dd_physical_block ump_memory_description;
	unsigned int buffer_size;

	buffer_size = fix->smem_len;
	ump_memory_description.addr = fix->smem_start ;
	ump_memory_description.size = PAGE_ALIGN(buffer_size);

	win->ump_wrapped_buffer[id] =
		ump_dd_handle_create_from_phys_blocks
		(&ump_memory_description, 1);

	if (ump_dd_meminfo_set(win->ump_wrapped_buffer[id], (void *)arg))
		return -ENOMEM;
...
static int s3c_fb_set_par(struct fb_info *info)
{
	...
	if (win_no == sfb->pdata->default_win) {
		...
		/* VIDTCON2 */
		data = VIDTCON2_LINEVAL(sfb->lcd_height - 1) |
		       VIDTCON2_HOZVAL(sfb->lcd_width - 1);
				...
				struct s3c_fb_user_plane_alpha user_alpha)
{
	...
	alpha_value = (((user_alpha.red & 0xf) << 8) |
			((user_alpha.green & 0xf) << 4) |
	...
	writel(data, regs + sfb->variant.wincon + (win_no * 4));
	writel(alpha_value, regs + VIDOSD_C(win_no, sfb->variant));
			...
			unsigned long arg)
{
	...
	case S3CFB_WIN_POSITION:
		if (copy_from_user(&p.user_window,
			...
			s3c_fb_set_window_position(info, p.user_window);
		}
		break;
	...
	case S3CFB_WIN_SET_PLANE_ALPHA:
		if (copy_from_user(&p.user_alpha,
			...
			s3c_fb_set_plane_alpha_blending(info, p.user_alpha);
		break;
	...
	case S3CFB_WIN_SET_CHROMA:
		if (copy_from_user(&p.user_chroma,
			...
			s3c_fb_set_chroma_key(info, p.user_chroma);
		break;
					 ...
					 struct s3c_fb_win *win)
{
	...
	phys->parts[0].start = fbi->fix.smem_start;

	win->s3c_fb_vcm[win->index].dev_vcm_res = vcm_map(sfb->s5p_vcm,
							  phys, 0);

	if (IS_ERR(win->s3c_fb_vcm[win->index].dev_vcm_res)) {
		vcm_unmap(win->s3c_fb_vcm[win->index].dev_vcm_res);
		kfree(phys);
		return -ENOMEM;
	}
#else
...
#if MALI_USE_UNIFIED_MEMORY_PROVIDER
#ifdef CONFIG_UMP_VCM_ALLOC
	ump_vcm.vcm = win->s3c_fb_vcm[win->index].dev_vcm;
	ump_vcm.vcm_res = win->s3c_fb_vcm[win->index].dev_vcm_res;
	ump_vcm.dev_id = win->index;
	arg = (unsigned int)&ump_vcm;

	ump_memory_description.addr = fbi->fix.smem_start;
	ump_memory_description.size = size;

	win->ump_wrapped_buffer[win->index] =
		ump_dd_handle_create_from_phys_blocks
		(&ump_memory_description, 1);
	if (ump_dd_meminfo_set(win->ump_wrapped_buffer[win->index], \
				(void *)arg))
		return -ENOMEM;
#else
	if (s3c_fb_ump_wrapper(&(fbi->fix), arg, win->index, win)) {
		dev_info(sfb->dev, \
				"[fb%d] : Wrapped UMP memory : %x\n"
				, win->index, \
				(unsigned int)(win->ump_wrapped_buffer));
		s3c_fb_unmap_video_memory(sfb, fbi);
		return -ENOMEM;
	}
#endif
#endif
...
#endif
		ump_dd_reference_release(win->ump_wrapped_buffer[win->index]);
		kfree(win->s3c_fb_vcm[win->index].dev_vcm_res);
	}
}
#else
static void s3c_fb_free_memory(struct s3c_fb *sfb, struct s3c_fb_win *win)
				      ...
				      struct s3c_fb_win **res)
{
	...
	*res = win;
	...
	if (win_no == sfb->pdata->default_win) {
		sfb->lcd_width = var->xres;
		sfb->lcd_height = var->yres;
	}

	s3c_fb_set_par(fbinfo);

	if (win_no == sfb->pdata->default_win)
		s3c_fb_draw_logo(fbinfo);
...
static int __devinit s3c_fb_probe(struct platform_device *pdev)
{
...
#if 0 /* temporarily disable lcd clock */
	...
	clk_enable(sfb->bus_clk);
#else
	ret = pd->enable_clk(pdev, &sfb->bus_clk);
	if (ret) {
		dev_err(dev, "failed to enable clock of FIMD\n");
		goto err_sfb;
	}
#endif
	...
	sfb->s5p_vcm = vcm_create_unified((SZ_64M), id, &s3c_fb_vcm_driver);
	if (IS_ERR(sfb->s5p_vcm))
		return PTR_ERR(sfb->s5p_vcm);
	if (vcm_activate(sfb->s5p_vcm))
		dev_info(sfb->dev, "[FIMD%d] : VCM activated", id);
#endif
...
/* writeback */
	fbfimd = kzalloc(sizeof(struct s3c_fb_fimd_desc), GFP_KERNEL);
	fbfimd->sfb_dev = sfb;

	v4l2_subdev_init(&sfb->sd, &s3c_fb_subdev_ops);
	sfb->sd.owner = THIS_MODULE;
	strcpy(sfb->sd.name, "s3c-fb");
	v4l2_set_subdevdata(&sfb->sd, pdev);
	dev_dbg(dev, "subdevice sucessful\n");
...
static int s3c_fb_runtime_resume(struct device *dev)
{
	...
	for (win_no = 0; win_no < S3C_FB_MAX_WIN; win_no++) {
		void __iomem *regs = sfb->regs;
		u32 wincon;
		wincon = readl(regs + sfb->variant.wincon + (win_no * 4));
		wincon &= ~WINCONx_ENWIN;
		writel(wincon, regs + sfb->variant.wincon + (win_no * 4));
	}
