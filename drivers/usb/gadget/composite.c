		...
		const char *buf, size_t size)
{
	...
	sscanf(buf, "%d", &value);
	printk(KERN_DEBUG "usb: enable_store %s %d\n", f->name, value);
...
void usb_composite_force_reset(struct usb_composite_dev *cdev)
{
	...
	if (cdev && cdev->gadget && cdev->gadget->speed != USB_SPEED_UNKNOWN) {
		...
		printk(KERN_DEBUG "usb: disconnect\n");
		usb_gadget_disconnect(cdev->gadget);
		msleep(10);
		printk(KERN_DEBUG "usb: connect\n");
		usb_gadget_connect(cdev->gadget);
	} else {
		printk(KERN_DEBUG "usb: skip force reset\n");
		spin_unlock_irqrestore(&cdev->lock, flags);
	}
}
		...
		enum usb_device_speed speed, void *buf, u8 type)
{
	...
	DBG(cdev, "config_buf configuration=%d, index_multi_conf=%d\n",
			config->bConfigurationValue, index_multi_conf);
	if (index_multi_conf > 0)
		DBG(cdev, "multi configuration=%d\n", index_multi_conf);
	if (config->cdev->product->multi_conf_functions
			[index_multi_conf] != 0) {
		product_functions = config->cdev->product->multi_conf_functions
			[index_multi_conf];
		product_num = config->cdev->product->
			num_multi_conf_functions[index_multi_conf];
		DBG(cdev, "set multi configuration=%d, size=%d\n",
				index_multi_conf, product_num);
	} else {
		product_functions = config->cdev->product->functions;
		product_num = config->cdev->product->num_functions;
		DBG(cdev, "set single configuration=%d, size=%d\n",
				config->cdev->multi_configuration, product_num);
	}
	...
	DBG(cdev, "config_buf config->cdev=0x%p\n", config->cdev);
	if (config->cdev->product == NULL) {
		printk(KERN_ERR "usb: config->cdev->product=NULL\n");
		return 0;
	}
	...
	for (i = 0; i < product_num; i++) {
		...
		list_for_each_entry(f, &config->functions, list) {
			...
			DBG(cdev,
		"i=%d config-cdev->product_num=%d, p_func=%s, c_func=%s\n",
				i, config->cdev->product->num_functions,
				product_function,  f->name);
			if (!strcmp(f->name, product_function)) {
				int index_intf = 0;
				int change_intf = 0;
				DBG(cdev, "set interface number (%s)\n",
						f->name);
				...
				if (f->disabled || !descriptors ||
						descriptors[0] == NULL)
					continue;

#ifdef CONFIG_USB_ANDROID_SAMSUNG_MTP
				/* Use vendor specific class for MAC OS */
				if (stringMode == MAC_REQUEST)
					index_multi_conf = 0;
				else
				/* Use still camera class for windows OS */
					index_multi_conf = 1;
#endif
				if (f->set_config_desc)
					f->set_config_desc(index_multi_conf);
				status = usb_descriptor_fillbuf(next, len,
				...
				while ((descriptor = *descriptors++) != NULL) {
					intf =
					(struct usb_interface_descriptor *)dest;
					if (
...
static int config_desc(struct usb_composite_dev *cdev, unsigned w_value)
{
	...
	if (w_value != 0) {
		...
		DBG(cdev, "single configuration = %d\n", w_value);
	}
...
static int count_configs(struct usb_composite_dev *cdev, unsigned type)
{
	...
	list_for_each_entry(c, &cdev->configs, list) {
		DBG(cdev, "%s :list usb_configuration=0x%p hs=%d,\
	...
	if (c->cdev->product->multi_conf_functions[0] != 0) {
		count = 0;
		if (hs) {
			if (!c->highspeed)
				return 1;
		...
		for (i = 0; i < MAX_MULTI_CONFIGURATION; i++) {
			if (cdev->product->multi_conf_functions[i] != 0) {
				DBG(cdev, "multi configuration=0x%p\n",
					cdev->product->multi_conf_functions
					[i]);
				count++;
			}
		}
	}
	DBG(cdev, "count config=%d\n", count);
	return count;
}
		...
		const struct usb_ctrlrequest *ctrl, unsigned number)
{
	...
	printk(KERN_DEBUG "usb: %s speed config #%d: %s\n",
		({ char *speed;
		switch (gadget->speed) {
	...
	DBG(cdev, "Initialize all interface c(0x%p)->bConfigurationValue=%d\n",
	...
	for (tmp = 0; tmp < MAX_CONFIG_INTERFACES; tmp++) {
		...
		printk(KERN_DEBUG "usb: interface[%d] %s\n", tmp, f->name);
		...
		void *buf, u16 language, int id)
{
	...
	if (id == 0) {
		...
		memcpy(os_desc->wData, &output.qwsignature,
				sizeof(os_string_descriptor_set));
 ...
 * you have to respond to host about 0x54 request.
 * Below handler must be moved in mtp function.
 */
char ms_descriptor[38] = {
	0x00, 0x00, 0x00, 0x01, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x4D, 0x54,	0x50, 0x00,
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_REQ_SET_CONFIGURATION:
		printk(KERN_DEBUG "usb: SET_CON v=%d\n", w_value);
	...
	case USB_REQ_SET_INTERFACE:
		...
		if (w_value && !f->set_alt)
			break;
		DBG(cdev,
		"Before SET_INTERFACE w_index=%d w_value=%d f->name=%s\n",
		w_index, w_value, f->name);
		value = f->set_alt(f, w_index, w_value);
		DBG(cdev,
		"After SET_INTERFACE w_index=%d w_value=%d f->name=%s\n",
		w_index, w_value, f->name);
		break;
...
composite_switch_work(struct work_struct *data)
{
	...
	printk(KERN_DEBUG "usb: %s con=%d sw.state=%d config=0x%p(%d)\n",
			__func__, cdev->connected, cdev->sw_connected.state,
			config, config ? config->bConfigurationValue : 0);
	spin_lock_irqsave(&cdev->lock, flags);
	if (cdev->connected != cdev->sw_connected.state) {
		connected = cdev->connected;
		spin_unlock_irqrestore(&cdev->lock, flags);
		switch_set_state(&cdev->sw_connected, connected);
		printk(KERN_DEBUG "usb: %s set con=%d\n", __func__, connected);
	} else {
		spin_unlock_irqrestore(&cdev->lock, flags);
	}
