...
static void req_put(struct mtpg_dev *dev, struct list_head *head,
...
static long  mtpg_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	case MTP_ONLY_ENABLE:
		printk(KERN_DEBUG "[%s:%d]mtp_function_enable [%d]\n",
		...
		if (dev->cdev && dev->cdev->gadget) {
#endif
			printk(KERN_DEBUG "[%s]B4 disconnect \tline = [%d]\n",
						 __func__, __LINE__);
			usb_composite_force_reset(dev->cdev);
		} else
			printk(KERN_DEBUG "usb: skip mtp reset\n");

		status = 10;
		printk(KERN_DEBUG "[%s][%d]MTP_ONLY_ENABLE error=0\n",
						__func__, __LINE__);
		the_mtpg->error = 0;
	...
	case SET_SETUP_DATA:
		...
		size = buf[0];
		DEBUG_MTPB("[%s]SET_SETUP_DATA size=%d line=[%d]\n",
						 __func__, size, __LINE__);
...
/*Test the switch */
	...
	if (rc != 0) {
		printk(KERN_ERR "Error in MTP usb_add_function Failed\n");
						...
						struct usb_configuration *c)
{
	...
	printk(KERN_DEBUG "[%s]mtp_function_config_changed\n", __func__);

	mtpg->function.bind = NULL;

	status = usb_add_function(c, &mtpg->function);
	if (status)
		printk(KERN_ERR "[%s]usb_add_function failed\n", __func__);

	status = usb_interface_id(c, &mtpg->function);
	if (status < 0) {
		printk(KERN_ERR "[%s]Error usb_string_id Failed\n", __func__);
		return status;
	}
...
void mtp_function_enable(int enable)
{
	...
	if (dev) {
		...
		if (enable) {
			printk(KERN_DEBUG "[%s] line %d fs and hs desc\n"
							, __func__, __LINE__);
		...
		printk(KERN_ERR "[%s] dev does not exist\n", __func__);
	}
}
