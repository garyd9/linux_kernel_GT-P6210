...
static int __init diag_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	diag->port.out = ep;
	ep->driver_data = cdev;	/* claim */
	printk(KERN_DEBUG "usb: %s in =0x%p, out =0x%p\n",
...
static void diag_unbind(struct usb_configuration *c, struct usb_function *f)
{
	if (gadget_is_dualspeed(c->cdev->gadget))
