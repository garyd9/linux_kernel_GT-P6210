...
{
	struct mtp_dev	*dev =
		container_of(data, struct mtp_dev, send_file_work);
	struct usb_composite_dev *cdev = dev->cdev;
	struct usb_request *req = 0;
	struct file *filp;
	loff_t offset;
...
{
	struct mtp_dev	*dev =
		container_of(data, struct mtp_dev, receive_file_work);
	struct usb_composite_dev *cdev = dev->cdev;
...
static int mtp_send_event(struct mtp_dev *dev, struct mtp_event *event)
{
	...
	if (dev->state == STATE_OFFLINE)
		return -ENODEV;
	/* unfortunately an interrupt request might hang indefinitely if the
	 * host is not listening on the interrupt endpoint,
