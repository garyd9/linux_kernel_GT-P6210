...
acc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	dev->cdev = cdev;
	printk(KERN_INFO "usb: acc_function_bind dev: %p\n", dev);
...
/*
	printk(KERN_DEBUG "acc_function_setup "
		...
		if (b_requestType == (USB_DIR_OUT | USB_TYPE_VENDOR)) {
			...
			if (b_request == ACCESSORY_GET_PROTOCOL) {
				*((u16 *)cdev->req->buf) = PROTOCOL_VERSION;
				...
				/* clear any strings left over from a previous
				   session */
				memset(dev->manufacturer, 0,
						sizeof(dev->manufacturer));
				memset(dev->model, 0, sizeof(dev->model));
				memset(dev->description, 0,
						sizeof(dev->description));
