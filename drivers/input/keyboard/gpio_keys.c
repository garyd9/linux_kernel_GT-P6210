	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ddata->n_buttons; i++)	{
		...
		ret = gpio_get_value(button->button->gpio);
		if (!ret)
			break;
	}

	printk(KERN_DEBUG "[Key] %s : %s\n", __func__,
		ret ? "Released" : "Pressed");

	return sprintf(buf, ret ? "RELEASE\n" : "PRESS\n");
}
...
static DEVICE_ATTR(sec_key_pressed, S_IRUGO, keyshort_test, NULL);
static DEVICE_ATTR(wakeup_keys, S_IWUSR, NULL, wakeup_enable);

static struct attribute *gpio_keys_test_attrs[] = {
	&dev_attr_sec_key_pressed.attr,
	&dev_attr_wakeup_keys.attr,
	NULL,
};

static struct attribute_group gpio_keys_test_attr_group = {
	.attrs = gpio_keys_test_attrs,
};
#endif
...
#ifdef CONFIG_SAMSUNG_INPUT
	dev_test  = device_create(sec_class, NULL, 0, ddata, "sec_key");
	if (IS_ERR(dev_test))
		pr_err("Failed to create device for the sysfs\n");
	else {
		error = sysfs_create_group(&dev_test->kobj,
			&gpio_keys_test_attr_group);
		if (error) {
			pr_err("Failed to create the test sysfs: %d\n", error);
			sysfs_remove_group(&dev_test->kobj,
				&gpio_keys_test_attr_group);
		}
	}
#endif
...
static int gpio_keys_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(&pdev->dev)) {
		for (i = 0; i < pdata->nbuttons; i++) {
			...
			button->isr_hook(button->code, false);
		}
	}
