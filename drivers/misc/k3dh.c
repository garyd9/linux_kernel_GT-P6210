...
static struct device *sec_k3dh_dev;
		       ...
		       const struct i2c_device_id *id)
{
...
#ifdef FACTORY_TEST
	res = sensors_register(accel_sensor_device, k3dh, accel_sensor_attrs, "accelerometer_sensor");
	if(res) {
		printk(KERN_ERR "%s: cound not register accelerometer sensor device(%d).\n", __func__, res);
	}


	dev_set_drvdata(accel_sensor_device, k3dh);
#else
	...
	err = device_create_file(dev_t, &dev_attr_raw_data);
	if (err < 0) {
...
err_cal_device_create:
	device_remove_file(dev_t, &dev_attr_raw_data);
