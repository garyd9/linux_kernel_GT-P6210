...
static ssize_t mpuirq_read(struct file *file, char *buf, size_t count, loff_t *ppos);
	...
	case MPUIRQ_SET_SELFTEST_IRQ_HANDLER:
		{
			...
			if (mpuirq_dev_data.irq > 0)
			{
				struct mldl_cfg *mldl_cfg = (struct mldl_cfg *) i2c_get_clientdata(mpuirq_dev_data.mpu_client);
				unsigned long flags;
				...
				res = request_irq(mpuirq_dev_data.irq,mpuirq_selftest_handler, flags, interface, &mpuirq_dev_data.irq);

				if (res) {
					printk("MPUIRQ_SET_SELFTEST_IRQ_HANDLER: cannot register IRQ %d\n", mpuirq_dev_data.irq);
				}
			}
		}    
		break;
		case MPUIRQ_SET_MPUIRQ_HANDLER:
			{
				...
				if (mpuirq_dev_data.irq > 0)
				{
					...
					res = request_irq(mpuirq_dev_data.irq,mpuirq_handler, flags, interface, &mpuirq_dev_data.irq);
				}
			}
			break;
		case MPUIRQ_GET_MPUIRQ_JIFFIES:
			{
				...
				if (copy_to_user((void *) arg, selftest_get_times, sizeof(selftest_get_times)))
				{
					memset( selftest_get_times,0,sizeof(selftest_get_times));
					return -EFAULT;
				}
