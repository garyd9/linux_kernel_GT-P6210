...
static int loadFile(void)
{
	...
	fp = filp_open("/mnt/sdcard/external_sd/s5k5bbgx.h", O_RDONLY, 0);
	if (IS_ERR(fp)) {
...
static int s5k5bbgx_check_sensor_status(struct v4l2_subdev *sd)
{
	...
	err = s5k5bbgx_read_reg(sd, 0x7000, 0x0132, &val_1);
	CHECK_ERR(err);
...
#if (1) //dslim
	printk(KERN_WARNING "%s:WARN: do nothing\n", __func__);
#else
					...
					struct v4l2_frmsizeenum *fsize)
{
	 ...
	 */
	/*DSLIM: if (state->req_fmt.priv == V4L2_PIX_FMT_MODE_CAPTURE) {
...
static int s5k5bbgx_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (state->sensor_mode == SENSOR_CAMERA) {
		...
		err = s5k5bbgx_write_regs_from_sd(sd,
			"s5k5bbgx_recording_60Hz_common");
#else
		err = s5k5bbgx_write_regs_from_sd(sd,
			"s5k5bbgx_recording_50Hz_common");
#endif
	}
#else
#if (0) //dslim
	if (state->sensor_mode == SENSOR_CAMERA) {
		...
		cam_info("load recording setting\n");
		err = s5k5bbgx_write_regs(sd, s5k5bbgx_recording_50Hz_common,
			sizeof(s5k5bbgx_recording_50Hz_common) / \
			sizeof(s5k5bbgx_recording_50Hz_common[0]));
	...
	cam_info("load 640x480 preview setting\n");

	err = s5k5bbgx_write_regs(sd, mode_preview_640x480,
				sizeof(mode_preview_640x480) / \
				sizeof(mode_preview_640x480[0]));
	msleep(50);
