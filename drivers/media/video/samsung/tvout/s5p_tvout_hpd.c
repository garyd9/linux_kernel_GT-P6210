...
static struct switch_dev hpd_switch;
...
static int s5p_hpd_ioctl(struct file *file,
...
static void s5p_hpd_kobject_uevent(void)
{
	...
	if (hpd_state) {
		if (last_uevent_state == -1 || last_uevent_state == HPD_LO) {
			...
			pr_info("[HDMI] HPD event -connect!!!\n");
		...
		if (last_uevent_state == -1 || last_uevent_state == HPD_HI) {
			...
			pr_info("[HDMI] HPD event -disconnet!!!\n");
...
#if defined(CONFIG_MACH_C1_REV01) || defined(CONFIG_MACH_C1_REV02) || defined(CONFIG_MACH_Q1_REV02)
