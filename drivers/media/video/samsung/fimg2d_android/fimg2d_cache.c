...
u32 g2d_check_pagetable(void * vaddr, unsigned int size, unsigned long pgd)
{
	...
	if (phys_to_virt(level_one_phy) < 0xc0000000) {
		FIMG2D_ERROR("Level1 page table mapping missed, missed address = 0x%x", phys_to_virt(level_one_phy));
	...
	if (phys_to_virt(level_two_phy) < 0xc0000000) {
		FIMG2D_ERROR("Level2 page table mapping missed, missed address = 0x%x", phys_to_virt(level_two_phy));
		return G2D_PT_NOTVALID;
	}
