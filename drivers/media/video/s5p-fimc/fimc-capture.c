...
static int buffer_prepare(struct vb2_buffer *vb)
{
	...
	if (!frame->fmt || vq->type != V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE)
		return -EINVAL;

	for (i = 0; i < frame->fmt->memplanes; i++) {
		unsigned long size = get_plane_size(frame, i);
		dbg("vb2_plane_size(%d) : %ld", i, size);
...
int fimc_register_capture_device(struct fimc_dev *fimc)
{
	...
	/* Default color format for 4BA image sensor */
	/* vid_cap->fmt.code = V4L2_MBUS_FMT_UYVY8_2X8; */
