...
void fimc_clk_disable(struct work_struct *work)
{
	struct fimc_dev *fimc = container_of(work, struct fimc_dev,
					     work_struct);

	if (!test_bit(ST_M2M_PEND, &fimc->state)) {
		if (!test_and_set_bit(ST_SUSPEND, &fimc->state)) {
#ifdef CONFIG_VIDEOBUF2_SDVMM
			vb2_sdvmm_suspend(fimc->alloc_ctx);
#endif
...
void fimc_clk_ctrl(struct fimc_dev *fimc)
{
	if (test_and_clear_bit(ST_SUSPEND, &fimc->state)) {
		pm_runtime_get_sync(&fimc->pdev->dev);
#ifdef CONFIG_VIDEOBUF2_SDVMM
		vb2_sdvmm_resume(fimc->alloc_ctx);
#endif
	} else {
		if (!test_bit(ST_M2M_PEND, &fimc->state)) {
			set_bit(ST_SUSPEND, &fimc->state);
#ifdef CONFIG_VIDEOBUF2_SDVMM
			vb2_sdvmm_suspend(fimc->alloc_ctx);
#endif
			pm_runtime_put_sync(&fimc->pdev->dev);
...
static int fimc_ctx_shut(struct fimc_ctx *ctx)
{
	...
	if (!fimc_m2m_pending(fimc) || (curr_ctx != ctx))
		return 0;

	set_bit(ST_M2M_SHUT, &ctx->shut);
	ret = wait_event_timeout(fimc->irq_queue,
			!test_bit(ST_M2M_SHUT, &ctx->shut),
...
static irqreturn_t fimc_isr(int irq, void *priv)
{
	...
	if (test_and_clear_bit(ST_M2M_PEND, &fimc->state)) {
		...
		if (src_vb && dst_vb) {
			...
			if (test_and_clear_bit(ST_M2M_SHUT, &ctx->shut))
				wake_up(&fimc->irq_queue);
		      ...
		      struct fimc_frame *frame, struct fimc_addr *paddr)
{
...
#ifdef CONFIG_VIDEOBUF2_SDVMM
	paddr->y = vb2_sdvmm_plane_dvaddr(vb, 0);
#else
	paddr->y = vb2_cma_plane_paddr(vb, 0);
#endif
	...
	if (frame->fmt->memplanes == 1) {
		...
		if (frame->fmt->memplanes >= 2)
			paddr->cb = vb2_sdvmm_plane_dvaddr(vb, 1);

		if (frame->fmt->memplanes == 3)
			paddr->cr = vb2_sdvmm_plane_dvaddr(vb, 2);
#else
		if (frame->fmt->memplanes >= 2)
			paddr->cb = vb2_cma_plane_paddr(vb, 1);

		if (frame->fmt->memplanes == 3)
			paddr->cr = vb2_cma_plane_paddr(vb, 2);
#endif
	}
...
static int fimc_buf_prepare(struct vb2_buffer *vb)
{
	...
	for (i = 0; i < frame->fmt->memplanes; i++) {
		...
		if (frame->cacheable)
			vb2_sdvmm_cache_flush(ctx->fimc_dev->alloc_ctx, vb, i);
	}
			  ...
			  struct v4l2_requestbuffers *reqbufs)
{
	...
	vb2_sdvmm_set_cacheable(ctx->fimc_dev->alloc_ctx, frame->cacheable);
...
#ifdef CONFIG_VIDEOBUF2_SDVMM
	src_vq->mem_ops = &vb2_sdvmm_memops;
#else
	src_vq->mem_ops = &vb2_cma_memops;
#endif
...
#ifdef CONFIG_VIDEOBUF2_SDVMM
	dst_vq->mem_ops = &vb2_sdvmm_memops;
#else
	dst_vq->mem_ops = &vb2_cma_memops;
#endif
...
static int fimc_m2m_open(struct file *file)
{
	...
	if (test_bit(ST_SUSPEND, &fimc->state)) {
		clear_bit(ST_SUSPEND, &fimc->state);
		pm_runtime_get_sync(&fimc->pdev->dev);
	}
...
/* FIXME */
	...
	fimc->state = ST_IDLE;
#ifdef CONFIG_VIDEOBUF2_SDVMM
	fimc->vcm_id = VCM_DEV_FIMC0 + pdev->id;
#endif
	...
	fimc->irq_workqueue = create_workqueue(workqueue_name);
	if (fimc->irq_workqueue == NULL) {
		dev_err(&pdev->dev, "failed to create workqueue for fimc\n");
		goto err_irq;
	}
	INIT_WORK(&fimc->work_struct, fimc_clk_disable);
	...
	fimc->alloc_ctx = vb2_sdvmm_init(&vb2_vcm, &vb2_cma, &vb2_drv);
	fimc_clk_ctrl(fimc);
#else
	fimc->alloc_ctx = vb2_cma_init(&fimc->pdev->dev, NULL, 0);
#endif
...
#ifdef CONFIG_VIDEOBUF2_SDVMM
	vb2_sdvmm_cleanup(fimc->alloc_ctx);
#else
	vb2_cma_cleanup(fimc->alloc_ctx);
#endif
