					 ...
					 u16 addr, u16 w_data)
{
	...
	do {
		...
		if (likely(ret == 1))
			break;
		msleep(POLL_TIME_MS);
		cam_err("%s: I2C err %d, retry %d.\n",
				__func__, ret, retry_count);
	} while (retry_count-- > 0);
	if (ret != 1) {
		cam_err("%s: I2C is not working.\n", __func__);
		return -EIO;
	}
			     ...
			     int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		err = s5k5ccgx_i2c_write_twobyte(client,
			(regs[i] >> 16), regs[i]);
		if (unlikely(err != 0)) {
			cam_err("%s: register write failed\n", __func__);
			return err;
		}
	}
			...
			const u32 list[], u32 size, char *name)
{
	...
	CHECK_ERR_N_MSG(err, "register write failed\n");
...
static int s5k5ccgx_set_ae_lock(struct v4l2_subdev *sd, s32 val)
{
	struct s5k5ccgx_state *state = to_state(sd);
	int err = 0;
	...
	case AE_UNLOCK:
		if (unlikely(!state->focus.ae_lock))
			return 0;
...
static int s5k5ccgx_set_awb_lock(struct v4l2_subdev *sd, s32 val)
{
	struct s5k5ccgx_state *state = to_state(sd);
	int err = 0;
	...
	case AWB_UNLOCK:
		if (unlikely(!state->focus.awb_lock))
			return 0;
...
/* PX: Set AE, AWB Lock */
static int s5k5ccgx_set_lock(struct v4l2_subdev *sd, s32 val)
{
	int err = -EIO;

	cam_trace("%s\n", val ? "on" : "off");

	err = s5k5ccgx_set_ae_lock(sd, val ? AE_LOCK : AE_UNLOCK);
	CHECK_ERR(err);
	err = s5k5ccgx_set_awb_lock(sd, val ? AWB_LOCK : AWB_UNLOCK);
	CHECK_ERR(err);
...
static int s5k5ccgx_return_focus(struct v4l2_subdev *sd)
{
	...
	if (state->scene_mode == SCENE_MODE_NIGHTSHOT)
		err = s5k5ccgx_set_from_table(sd, "af_night_normal_mode",
...
static int s5k5ccgx_af_start_preflash(struct v4l2_subdev *sd)
{
	...
	if (state->flash_on) {
		...
		if (count >= AE_STABLE_SEARCH_COUNT) {
...
/* PX: Set ISO */
static int s5k5ccgx_set_iso(struct v4l2_subdev *sd, s32 val)
{
	struct s5k5ccgx_state *state = to_state(sd);
	int err = -EINVAL;
...
#if !defined(CONFIG_VIDEO_S5K5CCGX_P2)
		if (unlikely(state->wide_mode)) {
			cam_dbg("%s: Restore capture setting\n", __func__);
			err = s5k5ccgx_set_from_table(sd, "restore_capture",
					&state->regs->restore_cap, 1, 0);
		}
#endif
...
static int s5k5ccgx_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAMERA_AE_LOCK_UNLOCK:
		err = s5k5ccgx_set_ae_lock(sd, ctrl->value);
		break;

	case V4L2_CID_CAMERA_AWB_LOCK_UNLOCK:
		err = s5k5ccgx_set_awb_lock(sd, ctrl->value);
		break;
